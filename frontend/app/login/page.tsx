...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem("auto_ide_token", data.access_token);
      router.push("/dashboard");
    } catch (err: any) {
      setError(err.message || "Something went wrong. Verify your credentials.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen relative flex items-center justify-center px-4 overflow-hidden">
      ...
      <div className="glass-card w-full max-w-md p-8 relative z-10 border border-white/10 flex flex-col gap-6">
        ...
        <div className="flex flex-col items-center text-center gap-2">
          ...
          <h2 className="text-2xl font-bold tracking-tight mt-2 text-white">Welcome back</h2>
          <p className="text-sm text-slate-400">Log in to manage your autonomous builds</p>
        </div>
